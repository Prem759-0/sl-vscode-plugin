...
import { ClientVersionInfo, WebsockClient, WebsockClientOptions } from "./websockclient";
...
export class JSONRPCClient extends WebsockClient implements JSONRPCInterface {
  ...
  >();
    ...
    private readonly clientInfo: ClientVersionInfo | undefined;
        ...
        switch (request.method) {
            ...
            case "system.getVersion":
                this.respondToJSONRPC(requestId, {
                    client_name: this.clientInfo?.name ?? "sl-vscode-plugin",
                    client_version: this.clientInfo?.version ?? "0.0.0",
                });
                break;
            case "system.status":
                this.respondToJSONRPC(requestId, {
